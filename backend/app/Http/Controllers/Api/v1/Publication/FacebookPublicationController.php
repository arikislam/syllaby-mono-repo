...
use App\Http\Requests\Publication\FacebookPublicationRequest;
...
class FacebookPublicationController extends Controller
{
    ...
    public function store(FacebookPublicationRequest $request, PublisherAction $action): JsonResponse|Response
    {
        try {
            $publication = $action->handle($request->validated(), SocialAccountEnum::Facebook->toString(), $request->publication, $request->channel);
            ...
            Log::error("Facebook Publication Failed - {$exception->getMessage()}");
