...
use App\Http\Requests\Publication\YoutubePublicationRequest;
...
class YoutubePublicationController extends Controller
{
    ...
    public function store(YoutubePublicationRequest $request, PublisherAction $action): JsonResponse|Response
    {
        try {
            $publication = $action->handle($request->validated(), SocialAccountEnum::Youtube->toString(), $request->publication, $request->channel);
            ...
            Log::debug("Unable to post on Youtube. Reason: {$exception->getMessage()}");
